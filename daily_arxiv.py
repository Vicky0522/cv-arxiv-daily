...
_github_search_rate_limit_reset = 0
...
def get_code_link(qword:str) -> str:
    ...
    """
    global _github_search_rate_limit_reset
    ...
    wait_seconds = _github_search_rate_limit_reset - time.time()
    if wait_seconds > 0:
        logging.info(f"github search rate limit hit, waiting {wait_seconds:.0f}s for reset")
        time.sleep(wait_seconds)

    try:
        r = requests.get(github_url, params=params, headers=headers, timeout=10)
        if r.headers.get("X-RateLimit-Remaining") == "0":
            _github_search_rate_limit_reset = int(r.headers.get("X-RateLimit-Reset", 0))
        results = r.json()
...
def get_official_code_link(paper_id:str) -> str:
    """
    @param paper_id: arxiv id, eg. 2108.09112v1
    @return paper_code from paperswithcode.com: string, if not found, return None
    """
    try:
...
def get_daily_papers(topic,query="slam", max_results=2):
    ...
    for result in client.results(search_engine):
        ...
        try:
            # source code link
            repo_url = get_official_code_link(paper_id)
            if repo_url is None:
        ...
        for keywords,v in json_data.items():
            ...
            for paper_id,contents in v.items():
                ...
                try:
                    repo_url = get_official_code_link(paper_id)
                    if repo_url is None:
                        repo_url = get_code_link(paper_title.strip('*'))
                    if repo_url is None:
